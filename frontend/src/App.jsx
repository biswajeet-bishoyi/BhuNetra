...
export default function App() {
  ...
  const handleConsentAccept = () => {
    ...
    clearConsent();
    localStorage.setItem('bhunetra_dpdp_consent', 'accepted');
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between selection:bg-amber-500 selection:text-slate-950">
      {/* DPDP Consent Dialog — must be accepted before app loads */}
      <ConsentDialog
        isOpen={!consentGiven}
      ...
      {consentDeclined ? (
        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center max-w-2xl mx-auto">
          ...
          <h1 className="text-2xl font-extrabold text-slate-100">Consent Declined</h1>
          <p className="text-sm text-slate-400 mt-3 max-w-md leading-relaxed">
            BhuNetra AI requires your consent under the DPDP Act 2023 to access land records. Without your
            consent, no data can be loaded or processed. You may revisit this page at any time to grant
            consent.
          </p>
          <button
            onClick={() => { setConsentDeclined(false); clearConsent(); }}
            className="mt-6 px-5 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs font-bold transition"
          >
            Grant Consent
          </button>
        </div>
